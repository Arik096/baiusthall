...
if(isset($_REQUEST['post_btn']))
{
 try
 {
  ...
  if(!isset($errorMsg))
  {
   $update_stmt=$db->prepare('UPDATE tbl_assistant_provost SET name=:name,position=:position,description=:description,phn_num=:phn_num,pic=:pic WHERE id=:id'); //sql update query
   ...
   if($update_stmt->execute())
   {
    ...
    header("location: assistant_provost.php");
   }
  }
 }

<?php
    ...
    if(isset($_REQUEST['btn_allot'])){
      ...
      elseif ($table=='17') {
        $sql="UPDATE tbl_male_hall_1 SET std_id='$std_id' WHERE floor_no='$floor' AND room_no='$room' AND bed_no='$bed'";
        $result=mysqli_query($connect,$sql);
      }
      if($result){
        header("location: new_std_application.php.php?msg=Alloted Success");
      }
